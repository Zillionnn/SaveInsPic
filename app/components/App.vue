<template>
  <Page>
    ...
    <AbsoluteLayout ref="rootLayout">
      ...
      <Label
        v-else
        text="The images will be displayed here"
        top="32"
        left="15"
      />
    </AbsoluteLayout>
      ...
      Clipboard.getText().then((content) => {
        ...
        httpModule
          .request({
            ...
            headers: {
              cookie:
                'ig_did=57D167EB-5DD8-456A-AEC8-EEA528C439C5; mid=XsVi7AALAAGgNbsvs-bs6fN3DvB1; csrftoken=7h2EOHDD6c6rFoWdga7VDBkq7ftIDZeL; ds_user_id=3597847967; sessionid=3597847967%3AVLLR1udD7p0lZo%3A20; shbid=11001; shbts=1591632683.6619833; urlgen="{"174.139.46.138": 35908}:1jjMJK:1qGr4DFWGfe7HpUYAaL89XSQ-mE"',
            },
          })
          .then(
            (response) => {
              ...
              console.log(endIdx);
              let jsonString = remain1.substring(
                startString.length,
                endIdx - 3
              );
              console.log(jsonString);
              console.log('>>', jsonString.substring(jsonString.length-29, jsonString.length))
              ...
              const mediaObj = shareObj.shortcode_media;
              console.log(mediaObj)

              // edge_sidecar_to_children
              // 多个媒体

              let multiMedia = mediaObj.hasOwnProperty("edge_sidecar_to_children");
              console.log('multiMedia', multiMedia)
              // TODO
              if (multiMedia) {
                for (let item of mediaObj.edge_sidecar_to_children.edges) {
                  if (item.node.is_video) {
                    ...
                    // 图片
                    let displayUrl = decodeURI(item.node.display_url);
                ...
                if (media.is_video) {
                  ...
                  // 图片
                  let displayUrl = decodeURI(media.display_url);
                  console.log(displayUrl);
              ...
              console.log('img Url list>>>', this.imgUrlList)
            },
            (e) => {}
          );
      });
